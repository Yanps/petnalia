...
/**
 * VetCard — marketplace search-result card for a veterinarian.
 * The system's flagship domain component.
 */
export function VetCard({
  name, specialty, photo, rating, reviews, distance,
  homeVisit = false, online = false, nextAvailable, price,
  verified = false, onSchedule, className = "", ...rest
}) {
  return (
    <Card interactive className={["vn-vetcard", className].filter(Boolean).join(" ")} {...rest}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", justifyContent: "space-between", gap: "var(--space-3)" }}>
            <div style={{ minWidth: 0 }}>
              ...
              <p style={{ margin: "2px 0 0", fontSize: "var(--small-size)", color: "var(--text-secondary)" }}>{specialty}</p>
            </div>
            {typeof rating === "number" && <Rating value={rating} count={reviews} />}
          </div>

          <div style={{ display: "flex", flexWrap: "wrap", gap: "var(--space-2)", marginTop: "var(--space-3)" }}>
            {homeVisit && <Badge variant="brand" icon="home">Home visit</Badge>}
      ...
      <div style={{
        ...
        <Button variant="primary" size="sm" iconLeft="calendar" onClick={onSchedule}>Schedule</Button>
      </div>
    </Card>
  );
}
